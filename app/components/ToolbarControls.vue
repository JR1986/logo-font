<template>
  <div class="bg-white border-b border-slate-200 px-3 py-2 flex items-center justify-start gap-2 shrink-0 dark:bg-slate-900 dark:border-slate-800 relative">
    ...
    <button
      ...
      class="flex items-center justify-between gap-2 w-32 sm:w-36 px-3 py-1.5 text-sm font-medium rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors dark:border-slate-700 dark:hover:bg-slate-800 shrink-0 bg-white dark:bg-slate-800"
  ...
  <Teleport to="body">
    ...
    <div 
      ...
      class="fixed z-50 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 w-72 max-w-[calc(100vw-32px)] overflow-hidden"
    ...
    >
      ...
      <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
        <div class="flex flex-wrap gap-1.5">
          <button 
            ...
            class="px-2 py-1 text-xs rounded-full transition-all duration-150"
            :class="selectedCategories.includes(category) 
              ? 'bg-blue-500 text-white shadow-sm' 
              : 'bg-white text-slate-600 border border-slate-200 hover:border-blue-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-600 dark:hover:border-blue-500'"
          >
            {{ category }}
          </button>
        </div>
      ...
      <div class="max-h-64 overflow-y-auto">
        <template v-for="(fontList, category) in fontCategories" :key="category">
          <!-- Category Header -->
          <div class="px-4 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-900/30 dark:text-slate-500 sticky top-0">
            {{ category }}
          </div>
          <!-- Font Items -->
          <button
            ...
            class="w-full px-4 py-2.5 text-left text-base transition-colors duration-100 flex items-center justify-between group"
