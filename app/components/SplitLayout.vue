<template>
  <div class="flex flex-1 overflow-hidden relative"> <!-- Added relative for positioning context if needed, though fixed is viewport based -->
    ...
    <aside 
      class="fixed inset-y-0 left-0 z-40 w-80 bg-white border-r border-slate-200 flex flex-col overflow-y-auto transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:z-0 md:w-96 md:shrink-0 shadow-xl md:shadow-none dark:bg-slate-900 dark:border-slate-800"
    ...
    <main class="flex-1 bg-slate-50 overflow-y-auto flex items-center justify-center p-4 md:p-12 relative pb-24 md:pb-12 dark:bg-slate-950"> <!-- Adjusted padding for mobile -->
      <div class="absolute top-6 left-6 text-xs font-bold text-slate-400 uppercase tracking-wider hidden md:block dark:text-slate-600">Live Preview Stage</div>
