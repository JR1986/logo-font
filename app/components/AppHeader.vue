<template>
  <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 z-10">
    ...
    <button 
      ...
      class="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-pink-600 transition-colors shadow-sm"
    >
      ...
      Saved Matches
      <span v-if="matchesCount > 0" class="ml-1 px-2 py-0.5 text-xs bg-pink-100 text-pink-700 rounded-full">
        {{ matchesCount }}
      </span>
    </button>
